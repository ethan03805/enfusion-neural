...
- Keep raw capture collections, profiles, machine paths and game assets in ignored runs/ or experiments/local/. The user has authorized selected documentation screenshots: publish only reviewed images under docs/media/ with hashes, source records and attribution. Keep model outputs labeled and preserve visible failures. Code licensing never grants rights to game content.
