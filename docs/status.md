...
The [600-frame paired exposure check](playable.md#paired-frame-exposure-stability) closes after 18.0 minutes. Native GPU replay exactly matches the retained snapshot; the independent CPU network matches, while the separate CPU composition failure remains recorded. Coarse added-exposure variation passes on the reserved segment at 83.79% valid-region coverage. This does not establish complete temporal or visibility acceptance. Numerical review finds **51,129 newly endpoint-clipped channel samples** across the sequence, a specific defect in the current compositor despite its luminance guard.

The [channel-preserving correction](playable.md#channel-preserving-correction) now passes its offline checks: **zero new clipped channels on all 600 frames and all fixed fixtures**, exact protected pixels and invalid-curve fallback, unchanged neural curves and passing coarse temporal limits. The original replay reproduces exactly. The source shader changes, while the download retains the previously verified build. The correction closes after 16.8 minutes; it supplies no substantial material or lighting gain.

Next: bounded live verification of the guard's captured output, controls and complete application cadence, then an updated package if it passes. The missing appearance target remains a separate blocker. The [roadmap](roadmap.md) fixes the next acceptance criteria. Blender studies remain [background research](research-history.md).
