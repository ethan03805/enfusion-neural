...
The [appearance evaluations](model-evaluation.md) retain rejected photo grades, costly restoration, native material/sky trials and coarse RGB depth. The latest IAT candidate fails appearance and its 10 ms call ceiling. Generic CC0 material references are useful background but are not aligned native-asset targets. No replacement model or improved training target is accepted.
...
Next: a bounded offline appearance-target feasibility check on one existing street frame, with strict identity and visibility rejection. A generated proposal will be labeled synthetic and cannot establish photographic ground truth or live performance. The [roadmap](roadmap.md) fixes the next check. Blender studies remain [background research](research-history.md).
