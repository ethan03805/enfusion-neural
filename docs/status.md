...
The subsequent three-view [moderate roughness comparison](model-evaluation.md#moderate-roof-candidates) rejects both candidates. The reserved view shows pale sheen and reduced slate contrast at 0.7 despite passing clipping and restoration checks. The original material remains preferable; no preset or training target is accepted.

Next: evaluate an RGB monocular-depth model against the retained offline collision samples and RX 7800 XT cost before attempting geometry-informed neural lighting. Inferred relative depth is not an engine buffer or metric geometry. The [roadmap](roadmap.md) gives the bound and remaining acceptance gates. Blender studies remain [background research](research-history.md).
