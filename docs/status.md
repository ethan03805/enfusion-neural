...
Prepared local scout configs are under `experiments/local/arma-diversity-scouts-v1/` for Beauregard, Arleville and a harbour. Their camera framing is unverified, and Arland towns do not replace the required larger city test. The image probe accepts `--config` and optional `--entities` for a vehicle/rifleman placement test using resource names found by the inventory. The placement script compiles; actual spawning and visibility remain untested. The resource probe accepts repeated `--query` arguments, so verified Everon world-resource discovery can begin with `--query Eden --query Everon` after the test render finishes. The new probe loader requires `--world-inventory` to bind an additional world to a completed native inventory; the default sequence contract still restricts callers to Arland.

A separate silent ScriptEditor inventory was tried without and with `-run` while the test rendered. Both addons compiled; both processes exited naturally without calling the plugin or returning resources. `evidence/enfusion-resource-inventory-silent-v1.json` retains the negative results. The distinct `probe_enfusion_resource_inventory.py` command must not be presented as a working inventory. No world or scene capture was requested. Use the existing WorldEditor resource probe after rendering, as above.
