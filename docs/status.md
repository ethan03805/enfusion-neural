...
- Thirty-three reviewed PNGs and twelve videos with source records, hashes and attribution. Scaling, label bands and compression are recorded for each video/poster; original source/model frame PNGs remain available.
...
The identity/file-return attempt in `experiments/local/image-bridge-identity-v1/` is also terminal. It timed out before its raw screenshot callback, leaving no source PNG or CPU output. The runtime evidence includes this fourth failure. All 46 CPU tests and the 20-page site build pass. The checked media manifest contains 33 PNGs and 12 videos; the theme and comparison-control checks also pass.
...
Everon now loads in the WorldEditor search-only probe. `experiments/local/everon-locations-v2/` holds the successful capture; `evidence/enfusion-everon-scout-v1.json` records it and the earlier material-traversal timeout. The inspected overview shows distant coast and sky. Its map query found 650 descriptors, but unnamed icons consumed the original 200-record output limit. The revised probe counts named locations separately. Use observed locations for ground-level city/interior scouts, then inspect actual images. `scripts/capture_enfusion_scout.py` keeps these source-only captures separate from the failed UI bridge and can place the observed vehicle/rifleman prefabs. Spawning and visibility still need runtime proof.
