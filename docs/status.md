...
The [menu/focus check](playable.md#menu-and-focus-check) closes in **15.1 minutes** using the unchanged extracted release. Bypass, focus-loss fallback, fresh-frame processing and F10 exit pass. Escape shows no menu with enhancement or bypass, and post-activation UI screenshots show only part of the scene. Complete menu/display recovery acceptance fails; causes remain unresolved. Working code, logs and original observations are retained.

Physical WASD/mouse interaction, complete menu/display recovery, interiors, scopes, adverse weather, HDR and semantic fallback remain unverified. Next is a source and window-geometry diagnosis before another live interaction trial. Empty moving-path ETW/DXGI display results remain explicit limitations; unchanged probes are closed. The missing aligned appearance target and substantial photorealistic gain remain unresolved. The [roadmap](roadmap.md) fixes the acceptance criteria; Blender remains [background research](research-history.md).
