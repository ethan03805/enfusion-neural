...
**Immediate next task:** establish the supported Enfusion feature and execution contract for the now-verified [native lighting graph](lighting-gpu.md). The graph passes all numerical controls and all 112 retained CPU comparisons; that does not supply engine inputs or output placement. Continue the bounded [camera lookup](color-lookup.md) transfer/sampling controls with independent colors and ramps, then reserved checks and motion/exposure tests. A lookup cannot implement the full model. Actual Arma model/reference coverage and complete-frame performance remain required before this goal is complete.
