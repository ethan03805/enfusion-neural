...
The replacement street route now passes the declared motion and repetition gates in standard/reduced/enhanced runs. Keep the failed fence pilots and world-loading crash. The subsequent DXGI statistics investigation is closed after its two declared probes: both return zero display counters, including the `DwmFlush` variant. Do not repeat those probes or the previous ETW search without new evidence that changes their feasibility.

The appearance investigation now identifies the existing `House_Village_E_1L02t` mesh and three uniquely matching roof/wall material containers. Their slate, plaster, brick and mask references are readable. The next bounded task is to verify its actual material binding and supported read-only access to the existing slate BCR/NMO and mask pixels. Require observed resource links, validated dimensions/channel meanings and preserved tile/door/window identity before defining a learned correction. Native texture inputs alone are not a higher-fidelity training target; state what reference improvement is actually available before training or replacing assets.
