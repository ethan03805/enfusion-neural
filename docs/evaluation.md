...
Correctness smoke tests use one dispatch with no warmup. Their timing fields are not performance evidence. Full benchmarks default to 10 warmups and 100 samples; `--warmup` and `--samples` make this explicit. Large repeated batches may exceed a slow or virtual device's limits. Retain failed runs and diagnose the device rather than treating a shortened correctness test as a successful full benchmark.
