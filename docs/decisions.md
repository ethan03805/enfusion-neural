...
## 044 · Limit exposure by source-channel headroom

Accepted 10 September 2026 for offline correction. Add one positive-gain ceiling derived from the brightest source channel and 254/255 output headroom before the existing feathered protection. Keep the common RGB multiplier, neural weights, strength, coordinates, negative bound and invalid-curve fallback unchanged. This leaves a rounding margin below a newly saturated RGB8 endpoint. The downloadable package is not replaced before live verification.

Reproduce the original native snapshot and every one of the 600 original output frames and curve arrays exactly. The corrected sequence has zero new endpoint-clipped channels, compared with 51,129 original channel samples. Every protected pixel remains identical to source and source-channel differences remain at most 15 RGB8 codes. A fixed 1,728-color atlas with constant bright curves exposes 1,511,657 original newly clipped channels and zero after correction. Dark, zero, out-of-range, NaN and infinity controls pass; zero and invalid inputs return exact source. Bright/dark CPU fixture differences have one-code maxima and mean errors 0.00444 / 0.00677, while the earlier gameplay CPU composition failure remains unresolved.

Both coarse temporal splits pass without changed flow, coverage, thresholds or reserved frames. The reserved p95 of frame median / frame p95 residual change is 0.071045 / 0.762951 RGB8 luminance codes at 83.79% median valid-region coverage. This remains a limited correspondence diagnostic, not full perceptual or semantic acceptance.

Inspect all four complete native source/original/guarded triplets, ten contact triplets and the poster. Six fixture pairs reduce to four unique full-size PNGs by exact file hashes; inspect each distinct image and retain all original files. The gameplay change affects only 162–334 pixels per fixed key, with no broad new artifact apparent in those samples. Blur and aliasing persist, and no photorealistic material/lighting gain is accepted. Close correction, review and evidence at 16.8 minutes within the original 30-minute bound, retaining two preparation failures. The native build, 79 Python tests and native contract checks pass. Require separate live capture, controls and application-cadence evidence before rebuilding the download.

## 045 · Release the guarded companion after a fixed live check

Accepted 10 September 2026. Freeze the already committed guard, exact executable/model/addon/settings and the original 30-minute clock before one private street session. The recorded simulation 30–60 window has 68.94 game and 59.97 companion application presents/s, with p95 intervals 19.51 / 30.61 ms. Exclude 1,818 internal capture records and the later snapshot/controls. Preserve twelve companion intervals above 33.3 ms, maximum 46.49 ms, all negative capture-age offsets and explicit recording overhead. No fallback or timeout occurs in the measured window.

The current native snapshot reproduces every curve and RGB8 channel exactly. Independent CPU curve error is at most 1.79 × 10⁻⁷, with zero new clipped channels, exact protected pixels and a maximum source-channel change of 15 codes. Sky hotkeys verify F8 bypass/resume, F9 modes 3 → 0 → 3 and F10 exit; the original game continues for five seconds. This does not verify physical WASD/mouse or display latency.

Review 34 chronological one-second samples, four native video keys and the complete source/output snapshot. Preserve the original 1,855 timestamps over 34 seconds; web encoding keeps each PTS exactly. Retain a failed rawvideo extraction caused by omitted VFR passthrough; discard its review keys and correct only the extraction in a new directory. Independently audit raw timeout/hide events after correcting the evaluator's timeout spelling; no acceptance result changes. Close review and the package audit at 17.9 minutes. Publish the verified 21-file ZIP with complete CMake sources, attribution and hashes; retain the previous ZIP and its measurements. The native source remains the preceding committed guard. Menu/focus transitions, physical input, full visibility/temporal acceptance and substantial photorealistic gain remain open.
