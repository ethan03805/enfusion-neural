...
def main():
    ...
    p.add_argument('--soak-seconds', type=int, default=0, help='Extended runtime check after the existing 30â€“60s path; remaining time is stationary. Zero keeps the original benchmark.')
    ...
    frozen_path = ROOT/'scenes/playable-live-guard-v1.json'
    ...
    if a.soak_seconds:
        manifest['soak_scope'] = 'Existing walking/turning path at simulation 30â€“60 seconds, then stationary town camera. Continuous runtime/cadence check; not ten minutes of movement, manual input or semantic acceptance.'
    ...
    ready = False; recorded = False; controls_ready = False; companion = None
    ...
    try:
        while time.monotonic() < deadline:
            ...
            if a.record and simulation >= 28 and not recorded:
                ...
                recorded = True
            if simulation >= 75 and not controls_ready:
