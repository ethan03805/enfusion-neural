...
def main():
    ...
    parser.add_argument('--startup', choices=('silent', 'module'), default='silent',
                        help='Silent startup, or normal ScriptEditor initialization with an initially hidden window')
    parser.add_argument('--extension', action='append', choices=('emat', 'edds', 'et', 'ent', 'conf'),
                        help='Restrict resource types; repeat for several types')
    ...
    if args.extension:
        probe, count = re.subn(r'array<string> extensions = \{[^\n]+\};',
                              'array<string> extensions = {' + ', '.join(json.dumps(e) for e in args.extension) + '};', probe)
        if count != 1:
            raise ValueError('Resource extension declaration changed')
    ...
    if args.startup == 'silent':
        argv.append('-wbsilent')
    argv += ['-wbModule=ScriptEditor', '-run', '-plugin=ENR_ResourceInventoryPlugin']
    record.update(argv=argv, queries=args.query, timeout_seconds=45,
                  ...
                  startup=args.startup, extensions=args.extension,
    ...
    with runner.project_lock(root):
        ...
        try:
            ...
            record.update(status='failed', error='Workbench resource inventory timed out after 45 seconds')
