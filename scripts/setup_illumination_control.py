"""Prepare the single selected sky-intensity source/change/reset control."""
import argparse
import json
...
def main():
    ...
    if digest(inventory / 'console.log') != selection['inventory_console_sha256']:
        raise ValueError('Changed selection inventory')
    initialize, _, doctor = lab_modules(a.lab_source)
    detected = doctor()
    if not detected['workbench_available']:
        raise ValueError('Workbench unavailable')
    initialize(out)
    source = ROOT / 'adapters/enfusion/probes/ENR_IlluminationControl.c'
    ...
    capture.write_bytes(script.replace(marker, marker.replace('  ELab_CaptureState.Armed = false;', '  if (!ENR_IlluminationControl.Step(world, timeslice)) return;\n  ELab_CaptureState.Armed = false;')).encode())
    ...
    write_json(out / 'setup.json', {'doctor': detected, 'selection': selection,
        'source_hashes': {path.relative_to(ROOT).as_posix(): digest(path) for path in [source, capture, config, selection_path, Path(__file__)]}})
    print(out)
