...
// Zero-DCE++ inference topology. Model weights are separate, licensed research
// data. NCHW FP32, zero convolution padding, bilinear RGB downsampling, no CPU
// frame copy.
class CurveNetwork {
  template <class T> using Ptr = Microsoft::WRL::ComPtr<T>;
  static void ok(HRESULT result) { winrt::check_hresult(result); }
  struct Buffer {
    Ptr<ID3D11Buffer> data;
    Ptr<ID3D11ShaderResourceView> srv;
    Ptr<ID3D11UnorderedAccessView> uav;
  };
  struct Params {
    UINT width, height, in_channels, out_channels, split, weight_offset,
        bias_offset, activation;
  };
...
[numthreads(8,8,1)] void pack(uint3 p:SV_DispatchThreadID) {
 ...
 curve[p.xy]=float4(first[at],first[n+at],first[2*n+at],1);
  ...
  Buffer buffer(UINT count, const float *initial = nullptr) {
    ...
    ok(device_->CreateUnorderedAccessView(b.data.Get(), nullptr, &b.uav));
    return b;
  }
  Ptr<ID3D11ComputeShader> shader(const char *entry) {
    Ptr<ID3DBlob> blob, error;
    auto hr = D3DCompile(source_, strlen(source_), "curve_network", nullptr,
                         nullptr, entry, "cs_5_0",
                         D3DCOMPILE_OPTIMIZATION_LEVEL3, 0, &blob, &error);
    if (FAILED(hr) && error)
      throw std::runtime_error(static_cast<char *>(error->GetBufferPointer()));
    ok(hr);
    Ptr<ID3D11ComputeShader> s;
    ok(device_->CreateComputeShader(blob->GetBufferPointer(),
                                    blob->GetBufferSize(), nullptr, &s));
    return s;
  }
  void dispatch(ID3D11ComputeShader *shader, Params p,
                ID3D11ShaderResourceView *a, ID3D11ShaderResourceView *b,
                ID3D11ShaderResourceView *weights,
                ID3D11ShaderResourceView *image, ID3D11UnorderedAccessView *out,
                ID3D11UnorderedAccessView *tex, UINT z) {
    ID3D11ShaderResourceView *srvs[] = {a, b, weights, image};
    ID3D11UnorderedAccessView *uavs[] = {out, tex};
    ctx_->UpdateSubresource(params_.Get(), 0, nullptr, &p, 0, 0);
    ctx_->CSSetConstantBuffers(0, 1, params_.GetAddressOf());
    ctx_->CSSetShaderResources(0, 4, srvs);
    ctx_->CSSetUnorderedAccessViews(0, 2, uavs, nullptr);
    ctx_->CSSetSamplers(0, 1, sampler_.GetAddressOf());
    ctx_->CSSetShader(shader, nullptr, 0);
    ctx_->Dispatch((width_ + 7) / 8, (height_ + 7) / 8, z);
    ID3D11ShaderResourceView *empty_srvs[4] = {};
    ID3D11UnorderedAccessView *empty_uavs[2] = {};
    ctx_->CSSetShaderResources(0, 4, empty_srvs);
    ctx_->CSSetUnorderedAccessViews(0, 2, empty_uavs, nullptr);
  }

public:
  CurveNetwork(ID3D11Device *device, ID3D11DeviceContext *ctx,
               const std::filesystem::path &path, UINT width = 320,
               UINT height = 180)
      : device_(device), ctx_(ctx), width_(width), height_(height) {
    if (!width || !height || width > 1280 || height > 720)
      throw std::runtime_error("Invalid network dimensions");
    std::ifstream f(path, std::ios::binary);
    char magic[4]{};
    f.read(magic, 4);
    if (std::memcmp(magic, "ZDC1", 4))
      throw std::runtime_error("Invalid curve weights header");
    for (int i = 0; i < 7; i++) {
      UINT ic = input_channels_[i], oc = output_channels_[i],
           count = 10 * ic + ic * oc + oc;
      std::vector<float> v(count);
      f.read(reinterpret_cast<char *>(v.data()), count * 4);
      if (!f)
        throw std::runtime_error("Truncated curve weights");
      for (float value : v)
        if (!std::isfinite(value))
          throw std::runtime_error("Nonfinite curve weight");
      weights_[i] = buffer(count, v.data());
      result_[i] = buffer(width_ * height_ * oc);
    }
    if (f.peek() != EOF)
      throw std::runtime_error("Unexpected trailing curve weights");
    image_ = buffer(width_ * height_ * 3);
    depth_ = buffer(width_ * height_ * 64);
    prepare_ = shader("prepare");
    depth_shader_ = shader("depthwise");
    point_shader_ = shader("pointwise");
    pack_ = shader("pack");
    D3D11_BUFFER_DESC d{};
    d.ByteWidth = sizeof(Params);
    d.Usage = D3D11_USAGE_DEFAULT;
    d.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    ok(device_->CreateBuffer(&d, nullptr, &params_));
    D3D11_SAMPLER_DESC sd{};
    sd.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
    sd.AddressU = sd.AddressV = sd.AddressW = D3D11_TEXTURE_ADDRESS_CLAMP;
    sd.MaxLOD = D3D11_FLOAT32_MAX;
    ok(device_->CreateSamplerState(&sd, &sampler_));
    ...
    ok(device_->CreateTexture2D(&td, nullptr, &curve_));
    ok(device_->CreateShaderResourceView(curve_.Get(), nullptr, &curve_srv_));
    ok(device_->CreateUnorderedAccessView(curve_.Get(), nullptr, &curve_uav_));
  }
  void run(ID3D11ShaderResourceView *image) {
    dispatch(prepare_.Get(), {width_, height_, 3, 3, 3, 0, 0, 0}, nullptr,
             nullptr, nullptr, image, image_.uav.Get(), nullptr, 1);
    run_prepared();
  }
  void set_input(const std::vector<float> &input) {
    if (input.size() != size_t(width_) * height_ * 3)
      throw std::runtime_error("Wrong curve input size");
    for (float value : input)
      if (!std::isfinite(value))
        throw std::runtime_error("Nonfinite curve input");
    ctx_->UpdateSubresource(image_.data.Get(), 0, nullptr, input.data(), 0, 0);
  }
  void run_prepared() {
    for (int i = 0; i < 7; i++) {
      ID3D11ShaderResourceView *a =
          i ? result_[i - 1].srv.Get() : image_.srv.Get();
      ID3D11ShaderResourceView *b = nullptr;
      UINT split = input_channels_[i];
      if (i >= 4) {
        a = result_[6 - i].srv.Get();
        b = result_[i - 1].srv.Get();
        split = 32;
      }
      UINT ic = input_channels_[i], oc = output_channels_[i];
      dispatch(depth_shader_.Get(),
               {width_, height_, ic, ic, split, 0, 9 * ic, 0}, a, b,
               weights_[i].srv.Get(), nullptr, depth_.uav.Get(), nullptr, ic);
      dispatch(point_shader_.Get(),
               {width_, height_, ic, oc, ic, 10 * ic, 10 * ic + ic * oc,
                i == 6 ? 2u : 1u},
               depth_.srv.Get(), nullptr, weights_[i].srv.Get(), nullptr,
               result_[i].uav.Get(), nullptr, oc);
    }
    dispatch(pack_.Get(), {width_, height_, 3, 3, 3, 0, 0, 0},
             result_[6].srv.Get(), nullptr, nullptr, nullptr, nullptr,
             curve_uav_.Get(), 1);
  ...
  void dump(const std::filesystem::path &path) {
    D3D11_BUFFER_DESC d{};
    result_[6].data->GetDesc(&d);
    d.Usage = D3D11_USAGE_STAGING;
    d.BindFlags = 0;
    d.CPUAccessFlags = D3D11_CPU_ACCESS_READ;
    d.MiscFlags = 0;
    d.StructureByteStride = 0;
    Ptr<ID3D11Buffer> staging;
    ok(device_->CreateBuffer(&d, nullptr, &staging));
    ctx_->CopyResource(staging.Get(), result_[6].data.Get());
    D3D11_MAPPED_SUBRESOURCE m{};
    ok(ctx_->Map(staging.Get(), 0, D3D11_MAP_READ, 0, &m));
    std::ofstream f(path, std::ios::binary);
    f.write(static_cast<char *>(m.pData), d.ByteWidth);
    ctx_->Unmap(staging.Get(), 0);
    if (!f)
      throw std::runtime_error("Curve output write failed");
  }
};
